function main_1DBEnstrophy_2024_v4_5(ascent,caseID,s_w,testnumber)

%{
[v4_5: generalize stochastic variants and add testnumber to testcase]
        ...
        for shiftT = 0:0 % "shiftT" <-> sensitivity testing || "tptest" <-> slingshot searching
            ...
            for enspt = ensstart:5:ensend % "enspt" <-> enstrophy point || "ss_shift" <-> shift slingshot initial data
                ...
                for timept = timestart:timeend 
                    ...
                    [testcase_max,testcase1,testcase0] = testcase_name(caseID,N,stepscale,ascent,tptest,pm,s,shiftT,ss_shift,0,Lip,cond,s_w,testnumber);
                    ...
                    for p = 1:ParamPoints % do in parallel
                        ...
                        while ( abs(J_step) > J_step_tol ) && ( ITER <= MAXITER )
                            ...
                            switch ascent
                                case {'RCGPR','RSCG','RMSCG','RDSCG','RJSCG','RSWMN','RSWM','RSWCGN','RSWCGJ','RSWGN','RSWG'}
                            ...
                            switch ascent
                                ...
                                case {'RSWG','RSWCGJ'}
                                    ...
                                    %}
                                    num = str2double(caseID);
                                    minITER = max(1,ITER-(num-1)); % RSLSCG (Stochastic-Leveraged 75%)
                                    rand_w = rand(1,ITER-minITER);
                                    rand_gradJ = zeros(1,length(gradJ));
                                    rand_direction = zeros(1,length(direction));
                                    for j = minITER:ITER-1
                                        % norm_save_gradJ = ( sum(abs(save_gradJ(j,:)).^2) + lambda^2*sum(abs(save_gradJ(j,:)).^2) );
                                        % rand_gradJ = rand_gradJ + rand_w(j-minITER+1)*(save_gradJ(j,:)/norm_save_gradJ);
                                        rand_gradJ = rand_gradJ + rand_w(j-minITER+1)*save_gradJ(j,:);
                                        rand_direction = rand_direction + rand_w(j-minITER+1)*save_direction(j,:);
                                    end
                                    % norm_save_gradJ = ( sum(abs(save_gradJ(ITER,:)).^2) + lambda^2*sum(abs(save_gradJ(ITER,:)).^2) );
                                    % rand_gradJ = rand_gradJ + (3*sum(rand_w))*(save_gradJ(ITER,:)/norm_save_gradJ);
                                    rand_gradJ = rand_gradJ + (s_w*sum(rand_w))*save_gradJ(ITER,:);
                                    rand_gradJ = rand_gradJ/((s_w+1)*sum(rand_w));
                                    % rand_direction = rand_direction + (2*sum(rand_w))*save_direction(ITER,:);
                                    % rand_direction = rand_direction/(3*sum(rand_w));
                                    rand_direction = direction;
                                case {'RSWGN','RSWCGN'}
                                    %{
                                    num = str2double(caseID);
                                    minITER = max(1,ceil((num-1)*ITER/num)); % RmSCG (Minor-Stochastic)
                                ...
                                case {'RSWM'}
                                    ...
                                    rand_gradJ = rand_gradJ + (s_w*sum(rand_w))*save_gradJ(ITER,:);
                                    rand_gradJ = rand_gradJ/((s_w+1)*sum(rand_w)); 
                                    rand_direction = rand_direction + (s_w*sum(rand_w))*save_direction(ITER,:);
                                    rand_direction = rand_direction/((s_w+1)*sum(rand_w));
                                    % rand_direction = direction;
                                case {'RSWMN'}
                                    %{
                                    num = str2double(caseID);
                                ...
                                case 'RDSCG'
                                    num = str2double(caseID);
                                    minITER = max(1,ITER-(num-1)); % RSLSCG (Stochastic-Leveraged 66%)
                                    rand_w = rand(1,ITER-minITER);
                                    rand_gradJ = zeros(1,length(gradJ));
                                    for j = minITER:ITER-1
                                        rand_gradJ = rand_gradJ + rand_w(j-minITER+1)*save_gradJ(j,:);
                                    end
                                    rand_gradJ = rand_gradJ + (2*sum(rand_w))*save_gradJ(ITER,:);
                                    rand_gradJ = rand_gradJ/(3*sum(rand_w)); 
                                case 'RJSCG'
                                    num = str2double(caseID);
                                    minITER = max(1,ITER-(num-1)); % RSLSCG (Stochastic-Leveraged 50%)
                                    rand_w = rand(1,ITER-minITER);
                                    rand_gradJ = zeros(1,length(gradJ));
                                    for j = minITER:ITER-1
                                        rand_gradJ = rand_gradJ + rand_w(j-minITER+1)*save_gradJ(j,:);
                                    end
                                    rand_gradJ = rand_gradJ + sum(rand_w)*save_gradJ(ITER,:);
                                    rand_gradJ = rand_gradJ/(2*sum(rand_w));                                  
                            end                            
                            ...
                            switch ascent
                                case {'RCGPR','RCGRMIL'}
                                    ...
                                    % phi = retraction(phi,E,K0,N); % retraction to constraint manifold
                                    psi = phi; % projection term
                                case {'RSCG','RMSCG','RDSCG','RJSCG','RSWMN','RSWM','RSWCGN','RSWCGJ'}  
                                    proj_gradJ = projection(rand_gradJ,psi,K0,N,lambda);
                                    psi_x = (2*pi*1i*K0).*psi; % derivative of projector term
                                    %
                                    trans_dir = projection(rand_direction,psi,K0,N,lambda) ...
                                        / ( sum(abs(psi).^2) + lambda^2*sum(abs(psi_x).^2) ); % vector transport of previous direction
                                    %}
                                    direction = proj_gradJ + beta*trans_dir; % direction towards optimal solution
                                    [tau,Jeval,iterJ] = optimize_tau(phi,direction,tau,E,K1,K0,T,nu,N,stepscale,Jeval,iterJ); % compute optimal step size 
