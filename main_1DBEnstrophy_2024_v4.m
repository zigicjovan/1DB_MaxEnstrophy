function main_1DBEnstrophy_2024_v4(ascent,caseID)
        ...
        for shiftT = 0:0 % "shiftT" <-> sensitivity testing || "tptest" <-> slingshot searching
            ...
            for enspt = ensstart:ensend % "enspt" <-> enstrophy point || "ss_shift" <-> shift slingshot initial data
                ...
                for timept = timestart:timeend 
                    ...
                    parfor p = 1:ParamPoints % do in parallel
                        ...
                        switch ascent
                            ...
                            case 'HB'
                                ...
                                Lip = max(real(ifft(gradJ))) ; % Lipschitz constant approximation ~ upper bound of quadratic growth
                                mu_bound = cond*Lip; % approximation ~ lower bound of quadratic growth
                                cond_inv = mu_bound/Lip; % inverse of condition number
                                alpha = ( ( 1 - sqrt(cond_inv) ) / ( 1 + sqrt(cond_inv) ) )^2;
                                beta = ( 2 / ( sqrt(Lip) + sqrt(mu_bound) ) )^2;
                                proj_gradJ = gradJ;%projection(gradJ,psi,K0,N,lambda); % projected gradient
                            ...
                            case 'NAG'
                                ...
                                Lip = 100*max(real(ifft(gradJ))) ; % Lipschitz constant approximation ~ upper bound of quadratic growth
                                mu_bound = cond*Lip; % approximation ~ lower bound of quadratic growth
                                cond_inv = mu_bound/Lip; % inverse of condition number
                                alpha = ( ( 1 - sqrt(cond_inv) ) / ( 1 + sqrt(cond_inv) ) )^2;
                                beta = ( 2 / ( sqrt(Lip) + sqrt(mu_bound) ) )^2;
                                proj_gradJ_int = gradJ;%projection(gradJ,psi,K0,N,lambda); % projected gradient
                        ...
                        K(ITER) = norm(uu);
                        switch ascent
                                case {'CGPR','CGRMIL','RCGPR','RCGRMIL'}
                                    step(ITER) = tau;
                                case {'RHB','RNAG'}
                                    step(ITER) = alpha;
                        end
                        ...
                        while abs(J_step) > J_step_tol && ITER <= MAXITER
                            ...
                            switch ascent
                                ...
                                case 'RNAG'
                                    ...
                                    mu_bound = cond*Lip; % approximation ~ lower bound of quadratic growth
                                    cond_inv = mu_bound/Lip; % inverse of condition number
                                    alpha = ( 1 - sqrt(cond_inv) ) / ( 1 + sqrt(cond_inv) ) ;
                                    beta = 1 / Lip ;
                                    %}
                            end
                            %%%
                            ...
                            switch ascent
                                ...
                                case {'RCGPR','RCGRMIL'}
                                    ...
                                    psi = phi; % projection term
                                case 'HB'
                                    proj_gradJ = gradJ;%projection(gradJ,psi,K0,N,lambda);
                                    psi_x = (2*pi*1i*K0).*psi; % derivative of projector term
                                    trans_dir = direction;%projection(direction,psi,K0,N,lambda)/( sum(abs(psi).^2) + lambda^2*sum(abs(psi_x).^2) ); % vector transport of previous direction
                                ...
                                case 'NAG'
                                    ...
                                    d_int = phi - phi_old;
                                    psi_x = (2*pi*1i*K0).*psi; % derivative of projector term
                                    trans_dir = d_int;%projection(d_int,psi,K0,N,lambda)/( sum(abs(psi).^2) + lambda^2*sum(abs(psi_x).^2) ); % vector transport of previous direction
                                    ...
                                    proj_gradJ_int = gradJ_int;%projection(gradJ_int,psi,K0,N,lambda); % projected gradient
                                    direction = alpha*proj_gradJ_int; % direction towards optimal solution
                                    phi_old = phi;
                                    phi = phi + direction; % iterative solution update
                                    psi = phi; % projection term
                                ...
                                case 'RNAG'
                                    ...
                                    d_int = phi - phi_old;
                                    psi_x = (2*pi*1i*K0).*psi; % derivative of projector term
                                    trans_dir = projection(d_int,psi,K0,N,lambda) ...
                                        /( sum(abs(psi).^2) + lambda^2*sum(abs(psi_x).^2) ); % vector transport of previous direction
                                    phi_int = phi + beta*trans_dir;
                                    [tvector_int,uField_int] = BurgersDS_Fourier(phi_int,K1,K0,T,nu,N,stepscale);
                                    Ntime_int = length(tvector_int);
